...
import { reservationsApi, propertiesApi, guestsApi } from '../utils/api';
...
interface ReservationDetailsModalProps {
  ...
  // ✅ NOVA PROP: Lista de propriedades para buscar nome sem API call
  properties?: Array<{ id: string; name: string; address?: any; bedrooms?: number; maxGuests?: number; type?: string }>;
}
...
export function ReservationDetailsModal({
  ...
  // ║                                                                               ║
  // ║  1. TODOS os hooks (useState, useEffect, useQuery, etc) DEVEM ficar ANTES    ║
  // ║     de qualquer "return" condicional (early return)                          ║
  // ║  2. NUNCA adicione "if (...) return" ANTES dos hooks                          ║
  // ║  3. O early return "if (!reservation) return null" DEVE ficar DEPOIS         ║
  // ║                                                                               ║
  // ║  Motivo: React exige que hooks sejam chamados na mesma ordem em cada render  ║
  // ║  Violação causa: "React error #310" ou "Rendered fewer hooks than expected"  ║
  // ╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  // Status do pagamento (usado no formulário na linha ~540)
  const [paymentStatus, setPaymentStatus] = useState<string>('pending');

  // ✅ BUSCAR PROPRIEDADE DA LISTA LOCAL PRIMEIRO (mais eficiente que API)
  const propertyFromList = properties.find(p => p.id === reservation?.propertyId);
  
  // ✅ BUSCAR DADOS DA PROPRIEDADE VIA API (fallback se não estiver na lista)
  const shouldFetchProperty = !!reservation?.propertyId && !propertyFromList;
  console.log('🔍 [PROPERTY QUERY] propertyId:', reservation?.propertyId, 'fromList:', !!propertyFromList, 'shouldFetch:', shouldFetchProperty);
  
  const { data: propertyDataFromApi, isLoading: propertyLoading, error: propertyError } = useQuery({
    queryKey: ['property', reservation?.propertyId],
    queryFn: async () => {
      if (!reservation?.propertyId) return null;
      console.log('🏠 Buscando propriedade via API:', reservation.propertyId);
      const response = await propertiesApi.get(reservation.propertyId);
      console.log('🏠 Resposta propriedade API:', response);
      return response.success ? response.data : null;
    },
    enabled: shouldFetchProperty,
    staleTime: 5 * 60 * 1000, // 5 minutos
  });
  
  // ✅ Priorizar dados da lista local, fallback para API
  const propertyData = propertyFromList || propertyDataFromApi;
  console.log('🏠 [PROPERTY DATA FINAL]', { propertyFromList: !!propertyFromList, propertyDataFromApi: !!propertyDataFromApi, propertyData });

  // ✅ BUSCAR DADOS DO HÓSPEDE
  const { data: guestData } = useQuery({
    queryKey: ['guest', reservation?.guestId],
    queryFn: async () => {
      if (!reservation?.guestId) return null;
      console.log('👤 Buscando hóspede:', reservation.guestId);
      const response = await guestsApi.get(reservation.guestId);
      console.log('👤 Resposta hóspede:', response);
      return response.success ? response.data : null;
    },
  ...
  // ✅ DADOS DO HÓSPEDE COM FALLBACK para guestName da reserva
  const guestDisplayName = guestData?.fullName || reservation?.guestName || 'Hóspede';
  const guestDisplayEmail = guestData?.email || '-';
  const guestDisplayPhone = guestData?.phone || '-';
  ...
  useEffect(() => {
    if (reservation) {
      ...
      const checkInDate = typeof reservation.checkIn === 'string' 
        ...
        : reservation.checkIn;
      const checkOutDate = typeof reservation.checkOut === 'string' 
        ? new Date(reservation.checkOut) 
        : reservation.checkOut;
      
      setEditCheckIn(checkInDate);
      setEditCheckOut(checkOutDate);
      // Sincronizar paymentStatus com a reserva se existir
      if (reservation.payment?.status) {
        setPaymentStatus(reservation.payment.status);
      }
    }
  }, [reservation]);
  // === FIM DO BLOCO DE HOOKS ===

  // ✅ Early return DEPOIS de todos os hooks (evita React error #310)
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.success('Reserva cancelada com sucesso!', {
          description: `Valor a devolver: R$ ${data.refundAmount?.toFixed(2) || reservation.pricing?.total?.toFixed(2) || '0.00'}`
        });
  ...
  // ✅ FIX: O pricing.total JÁ inclui tudo (diárias + taxas + fees)
  // Não somar novamente cleaningFee/serviceFee
  const totalAmount = reservation.pricing?.total ?? reservation.price ?? 0;
  const baseStayAmount = reservation.pricing?.baseTotal ?? (totalAmount - (reservation.pricing?.cleaningFee ?? 0) - (reservation.pricing?.serviceFee ?? 0));
  const pricePerNight = reservation.pricing?.pricePerNight ?? (nights > 0 ? baseStayAmount / nights : 0);
  const cleaningFee = reservation.pricing?.cleaningFee ?? 0;
  ...
  // DEBUG: Log dos cálculos financeiros
  console.log('💰 CÁLCULOS FINANCEIROS:', {
    'reservation.pricing': reservation.pricing,
            ...
            <TabsContent value="overview" className="space-y-6 mt-0">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div>
                      <div className="font-medium mb-2">{propertyData?.name || `Propriedade #${reservation.propertyId?.slice(0, 8) || '-'}`}</div>
                      <div className="text-sm text-gray-600">
                        {propertyData?.type || 'Imóvel'}
                        {propertyData?.bedrooms ? ` • ${propertyData.bedrooms} quartos` : ''}
                        {propertyData?.maxGuests ? ` • ${propertyData.maxGuests} hóspedes` : ''}
                      </div>
                      <div className="text-sm text-gray-500 flex items-center gap-1 mt-1.5">
                        <MapPin className="w-3.5 h-3.5" />
                        {propertyData?.address ? `${propertyData.address.neighborhood || ''}, ${propertyData.address.city}` : '-'}
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-medium">{reservation.guests?.adults ?? '-'}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-600">Crianças</span>
                          <span className="font-medium">{reservation.guests?.children ?? '-'}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-600">Pets</span>
                          <span className="font-medium">{reservation.guests?.pets ?? '-'}</span>
                        </div>
                        <Separator />
                        <div className="flex justify-between items-center">
                          <span className="font-medium">Total</span>
                          <span className="font-medium">{reservation.guests?.total ?? '-'} pessoas</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {reservation.notes ? (
                    ...
                    <div className="text-sm text-gray-500 italic">Nenhuma observação registrada.</div>
                  )}
            ...
            <TabsContent value="guest" className="space-y-6 mt-0">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-lg">{guestDisplayName}</div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-2 text-gray-700">
                          ...
                          <div>
                            ...
                            <span className="text-sm">{guestDisplayEmail !== '-' ? guestDisplayEmail : 'Não informado'}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 text-gray-700">
                          ...
                          <div>
                            ...
                            <span className="text-sm">{guestDisplayPhone !== '-' ? guestDisplayPhone : 'Não informado'}</span>
                          </div>
                        </div>
                        {guestData?.cpf && (
                          <div className="flex items-center gap-2 text-gray-700">
                            ...
                            <div>
                              ...
                              <span className="text-sm">{guestData.cpf}</span>
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-base">Histórico do Hóspede</CardTitle>
                    </CardHeader>
                    <CardContent>
                      {guestData?.stats ? (
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{guestData.stats.totalReservations}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Total de noites</span>
                            <span className="font-medium">{guestData.stats.totalNights}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Valor total gasto</span>
                            <span className="font-medium">R$ {guestData.stats.totalSpent?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                          </div>
                          {guestData.isBlacklisted && (
                            <>
                              <Separator />
                              <div className="flex justify-between items-center">
                                <span className="font-medium">Status</span>
                                <Badge variant="destructive">Na lista negra</Badge>
                              </div>
                            </>
                          )}
                        </div>
                      ...
                      <div className="space-y-3">
                        {paymentStatus === 'paid' ? (
                          ...
                          <div className="p-3 border border-yellow-200 bg-yellow-50 rounded-lg">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-medium text-yellow-900">Pagamento Pendente</span>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="p-3 border rounded-lg">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <span className="text-sm text-gray-600">{format(new Date(reservation.checkIn), 'dd MMM yyyy', { locale: ptBR })}</span>
                          </div>
                          <div className="text-sm text-gray-600">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="border rounded-lg p-6 space-y-6 bg-white">
                    ...
                    <div className="flex justify-between">
                      <div>
                        <h3 className="font-medium text-lg mb-2">Rendizy</h3>
                        <div className="text-sm text-gray-600 space-y-1">
                          <div>Sistema de Gestão de Reservas</div>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        <div><strong>Nome:</strong> {guestDisplayName}</div>
                        <div><strong>E-mail:</strong> {guestDisplayEmail !== '-' ? guestDisplayEmail : 'Não informado'}</div>
                        <div><strong>Telefone:</strong> {guestDisplayPhone !== '-' ? guestDisplayPhone : 'Não informado'}</div>
                        {guestData?.cpf && <div><strong>CPF:</strong> {guestData.cpf}</div>}
                      </div>
                    </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-600 mb-2">Em breve</h3>
                    <p className="text-sm text-gray-500 max-w-md">
                      O histórico detalhado de ações e eventos da reserva estará disponível em uma próxima atualização.
                    </p>
                  </div>
                </CardContent>
              </Card>
