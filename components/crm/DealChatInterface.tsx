...
import { Deal, DealSource, DealMessage } from '../../types/crm';
...
export function DealChatInterface({ deal }: DealChatInterfaceProps) {
  // Mock messages - será substituído por API
  const getMockMessages = (): DealMessage[] => [
    {
      id: '1',
      dealId: deal.id,
      contactId: '1',
      content: 'Hi Ana, sending over the proposal.',
      sender: 'USER',
      source: deal.source,
    ...
    },
    {
      ...
      dealId: deal.id,
      contactId: '1',
      content: 'Received. Looks good, lets discuss tomorrow.',
      sender: 'CONTACT',
      source: deal.source,
  ...
  const [messages, setMessages] = useState<DealMessage[]>(getMockMessages());
  ...
  const handleSendMessage = async () => {
    ...
    // Otimistic update
    const tempMessage: DealMessage = {
      id: `temp-${Date.now()}`,
      dealId: deal.id,
      contactId: deal.contactId || '1',
    ...
    // Enviar para API
    try {
      const response = await dealsApi.sendMessage({
        dealId: deal.id,
        contactId: deal.contactId || '1',
        content: messageContent,
        source: deal.source,
