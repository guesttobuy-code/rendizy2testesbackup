...
│  - anuncios_drafts (rascunhos com JSONB flexível)          │
│  - anuncios_published (normalizado para busca rápida)       │
...
**anuncios_drafts** - Rascunhos com salvamento campo-por-campo
```sql
{
  id: uuid,
...
```sql
{
  id: uuid,
  title: text,
  tipo_local: varchar,      -- extraído para índice
  cidade: text,             -- extraído para filtros
  data: jsonb               -- backup completo
}
```
...
**R**: Não! A migration cria tabelas novas (`anuncios_drafts`). Sua tabela `anuncios_ultimate` permanece intacta. Podemos migrar dados depois.
