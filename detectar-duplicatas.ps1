# Script para detectar anúncios duplicados em properties
...
Write-Host "`n🔍 VERIFICANDO DUPLICATAS EM properties`n" -ForegroundColor Cyan
...
foreach ($id in $testIds) {
    $anuncio = Invoke-RestMethod -Uri "$SUPABASE_URL/rest/v1/properties?id=eq.$id&select=id,title,created_at" -Headers $h
    if ($anuncio) {
