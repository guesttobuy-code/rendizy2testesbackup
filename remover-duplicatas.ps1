...
$todos = Invoke-RestMethod -Uri "$URL/rest/v1/properties?select=id,title,data" -Headers @{ "apikey" = $KEY }
...
foreach ($testId in $testIds) {
    ...
    if ($duplicatas.Count -gt 0) {
        ...
        foreach ($dup in $duplicatas) {
            ...
            try {
                $delUrl = "$env:URL/rest/v1/properties?id=eq.$($dup.id)"
...
$todosDepois = Invoke-RestMethod -Uri "$URL/rest/v1/properties?select=id" -Headers @{ "apikey" = $KEY }
