...
> Nota (atual): o sistema de anúncios usa **tabela única** `properties`.
...
- (Legado) Foi experimentado fallback direto ao REST quando a função `properties/lista` retornava vazia, para destravar ambientes antigos.
	- Atual: **não** usar fallback REST para anúncios no app; a tabela canônica é `properties` e o acesso deve ser via Edge com tenancy.
