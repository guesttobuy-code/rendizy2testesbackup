...
### 7.1 Transição Rascunho → Publicado (no mesmo registro)

```sql
CREATE OR REPLACE FUNCTION public.publish_anuncio(p_anuncio_id uuid)
RETURNS uuid AS $$
DECLARE
  v_completion int;
BEGIN
...
- [ ] (Opcional) Criar tabelas auxiliares `anuncios_versions`, `anuncios_pending_changes`
