...
## 🔴 SEÇÃO 0: ADRs CRÍTICAS (ARCHITECTURAL DECISION RECORDS)

> **⚠️ LEITURA OBRIGATÓRIA PARA TODA IA QUE TRABALHAR NESTE PROJETO**
...
### ADR #2: Referência Técnica de Webhooks StaysNet

📄 **Documento**: [ADR_STAYSNET_WEBHOOK_REFERENCE.md](./ADR_STAYSNET_WEBHOOK_REFERENCE.md)
...
1. **NUNCA** criar pastas novas em `supabase/functions/` sem permissão explícita do usuário
2. **SEMPRE** adicionar novas funcionalidades como **rotas** em `rendizy-server`
3. **SEMPRE** verificar se já existe uma rota antes de criar
...
| Data | Mudança | Motivo |
|------|---------|--------|
| 2026-01-18 | Adicionada Seção 0: ADRs Críticas | Documentar arquitetura de Edge Functions |
| 2026-01-18 | Criada ADR_EDGE_FUNCTIONS_ARQUITETURA_CENTRALIZADA | Incidente de 20 dias sem webhooks |
| 2026-01-18 | Criada ADR_STAYSNET_WEBHOOK_REFERENCE | Referência técnica de webhooks |
...
| Documento | Descrição |
|-----------|-----------|
| [ADR_EDGE_FUNCTIONS_ARQUITETURA_CENTRALIZADA.md](./ADR_EDGE_FUNCTIONS_ARQUITETURA_CENTRALIZADA.md) | **ADR**: Arquitetura centralizada - UMA função, múltiplas rotas |
| [ADR_STAYSNET_WEBHOOK_REFERENCE.md](./ADR_STAYSNET_WEBHOOK_REFERENCE.md) | **ADR**: Referência técnica de webhooks StaysNet |
