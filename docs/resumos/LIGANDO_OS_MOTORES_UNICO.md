...
- **Regra**: todo código sai desta pasta principal → push para `rendizy2testesbackup` (ramo `main`) → validar em Vercel → após OK explícito, fazer push para `Rendizyoficial`.
- **Evitar divergências**: sempre `git pull` em `main` antes de começar; não trabalhar em cópias antigas nem outras pastas.
...
- Automação ou IA deve sempre verificar a existência do arquivo-alvo antes de criar um novo; ao detectar que o nome já existe, deve atualizar o arquivo canônico em vez de gerar outra cópia. Qualquer violação bloqueia o push e deve ser tratada como incidente crítico.
...
- Revisores devem recusar mudanças que introduzam arquivos duplicados. Cite explicitamente esta regra e exija consolidação antes de seguir.

- Hooks / CI recomendados:
  - Pré-commit: usar `husky` ou um hook Git simples que execute `npm run check-duplicates` antes do commit.
  - CI: rodar `npm run check-duplicates` e `npx -p typescript tsc --noEmit` como etapas bloqueantes antes do push.
