...
echo     IF FOUND THEN
echo       RETURN QUERY SELECT anuncios_ultimate.id, anuncios_ultimate.data FROM public.anuncios_ultimate WHERE anuncios_ultimate.id = v_existing_annuncio;
echo       RETURN;
echo     END IF;
echo   END IF;
echo.
...
echo     INSERT INTO public.anuncios_ultimate (organization_id, user_id, data) VALUES (p_organization_id, p_user_id, jsonb_build_object(p_field, p_value)) RETURNING public.anuncios_ultimate.id INTO v_id;
echo   ELSE
echo     v_id := p_anuncio_id;
echo     UPDATE public.anuncios_ultimate
echo       SET data = jsonb_set(coalesce(data, '{}'::jsonb), ARRAY[p_field], p_value, true)
echo       WHERE id = v_id;
echo     IF NOT FOUND THEN
echo       INSERT INTO public.anuncios_ultimate (id, organization_id, user_id, data) VALUES (v_id, p_organization_id, p_user_id, jsonb_build_object(p_field, p_value));
echo     END IF;
echo   END IF;
echo.
...
echo.
echo   RETURN QUERY SELECT anuncios_ultimate.id, anuncios_ultimate.data FROM public.anuncios_ultimate WHERE anuncios_ultimate.id = v_id;
echo END;
