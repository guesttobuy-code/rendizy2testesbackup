...
## ⚠️ ADRs CRÍTICAS - LEITURA OBRIGATÓRIA

### ADR #1: Arquitetura de Edge Functions
📄 **Leia:** [docs/ADR_EDGE_FUNCTIONS_ARQUITETURA_CENTRALIZADA.md](../docs/ADR_EDGE_FUNCTIONS_ARQUITETURA_CENTRALIZADA.md)

| ✅ PERMITIDO | ❌ PROIBIDO |
|-------------|-------------|
| Adicionar rotas em `rendizy-server` | Criar novas pastas em `supabase/functions/` |
| Usar `rendizy-public` para sites públicos | Importar de `rendizy-server` em outras functions |
| Deploy com `npm run deploy:server` | "Separar para organizar" |

**CONTEXTO:** Em Dezembro 2025, uma IA criou Edge Functions separadas que ficaram desincronizadas por 20+ dias, causando perda de webhooks e reservas.

### ADR #2: Referência de Webhooks StaysNet
📄 **Leia:** [docs/ADR_STAYSNET_WEBHOOK_REFERENCE.md](../docs/ADR_STAYSNET_WEBHOOK_REFERENCE.md)
