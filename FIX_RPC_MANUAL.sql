...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    IF FOUND THEN
      RETURN QUERY SELECT t.id AS id, t.data AS data FROM public.anuncios_ultimate t WHERE t.id = v_existing_annuncio;
      RETURN;
    END IF;
  END IF;
  ...
  RETURN QUERY SELECT t.id AS id, t.data AS data FROM public.anuncios_ultimate t WHERE t.id = v_id;
END;
