...
// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  ⚠️  ATENÇÃO: BACKEND COMPARTILHADO - MULTI-SITE                             ║
// ║══════════════════════════════════════════════════════════════════════════════║
// ║  Este arquivo é usado por TODOS os sites de clientes:                        ║
// ║  - medhome, suacasamobiliada, e qualquer site futuro                         ║
// ║                                                                              ║
// ║  ANTES DE QUALQUER MUDANÇA:                                                  ║
// ║  1. Verifique se a mudança é BACKWARD COMPATIBLE                             ║
// ║  2. Teste em TODOS os sites ativos (não apenas o que você está debugando)    ║
// ║  3. Atualize o prompt de sites se necessário:                                ║
// ║     → _PROMPT_HANDOFF_2026_01_06_SITES_PROMPT.md                             ║
// ║     → components/client-sites/catalog.ts                                     ║
// ║  4. Documente a mudança em docs/changelogs/                                  ║
// ║                                                                              ║
// ║  Sites afetados por este backend:                                            ║
// ║  @site medhome     - Produção - https://medhomecelsosobral.vercel.app        ║
// ║  @site suacasa     - Produção - https://rendizy-site-sua-casa-mobiliada...   ║
// ║                                                                              ║
...
// ============================================================
// Helper: Resolve client site by subdomain with fallback to org slug
// Used by all /client-sites/api/:subdomain/* endpoints for consistency
// ============================================================
// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  🔧 FUNÇÃO CRÍTICA: resolveClientSite()                                      ║
// ║══════════════════════════════════════════════════════════════════════════════║
// ║  Esta função é usada por TODOS os endpoints /client-sites/api/:subdomain/*   ║
// ║  Qualquer mudança aqui afeta TODOS os sites de clientes.                     ║
// ║                                                                              ║
// ║  Comportamento:                                                              ║
// ║  1. Primeiro tenta match EXATO por subdomain (comportamento original)        ║
// ║  2. Se não encontrar, tenta fallback por org slug normalizado                ║
// ║                                                                              ║
// ║  BACKWARD COMPATIBLE: Sites existentes continuam funcionando.                ║
// ║  Adicionado em: 2026-01-18 para corrigir "Site não encontrado" em alguns     ║
// ║  sites onde o subdomain não correspondia exatamente ao registro.            ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
type ResolvedClientSite = {
  organization_id: string;
  subdomain: string;
  is_active: boolean;
};
...
async function resolveClientSite(
  ...
  subdomain: string
): Promise<ResolvedClientSite | null> {
  // Attempt 1: Direct subdomain match
  ...
  if (!sqlError && sqlSite) {
    return sqlSite as ResolvedClientSite;
  }

  // Fallback: resolve by organization slug (normalized)
  ...
  const orgMatch = (orgs || []).find((org: { id: string; slug: string | null }) =>
  ...
  if (orgMatch?.id) {
    const { data: siteByOrg } = await supabase
      ...
      .maybeSingle();
    if (siteByOrg) {
      return siteByOrg as ResolvedClientSite;
    }
  }
...
clientSites.get("/api/:subdomain/properties", async (c: Context) => {
  try {
    ...
    // Use centralized resolver with org slug fallback
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
    ...
    const organizationId = resolvedSite.organization_id;
...
clientSites.get("/api/:subdomain/site-config", async (c: Context) => {
  try {
    ...
    // Resolve site with fallback to org slug
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
...
clientSites.get("/api/:subdomain/calendar", async (c: Context) => {
  try {
    ...
    // Validate site + org (with fallback to org slug)
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
    ...
    const orgId = resolvedSite.organization_id;
    ...
    // Return flat array: { success: true, data: CalendarDay[] }
    // Front-end expects result.data to be the array directly
    return c.json(
      { success: true, data: daysResult },
      200,
      withCorsHeaders({
...
clientSites.post("/api/:subdomain/calculate-price", async (c: Context) => {
  try {
    ...
    // Validate site (with fallback to org slug)
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
...
clientSites.post("/api/:subdomain/reservations", async (c: Context) => {
  try {
    ...
    // Validate site exists (with fallback to org slug)
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
...
clientSites.get("/api/:subdomain/payment-methods", async (c: Context) => {
  try {
    ...
    // Load site to get organization (with fallback to org slug)
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
...
clientSites.post("/api/:subdomain/checkout/session", async (c: Context) => {
  try {
    ...
    // Validate site exists and get organization (with fallback to org slug)
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
...
clientSites.get("/api/:subdomain/reservations/:reservationId/status", async (c: Context) => {
  try {
    ...
    // 1. Buscar site usando resolver (suporta org slug como fallback)
    const resolvedSite = await resolveClientSite(supabase, subdomain);
    if (!resolvedSite) {
      return c.json(
        { success: false, error: "Site não encontrado" },
