// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  ⚠️  ATENÇÃO - LEITURA OBRIGATÓRIA ANTES DE MODIFICAR ESTE ARQUIVO  ⚠️       ║
// ╠══════════════════════════════════════════════════════════════════════════════╣
// ║                                                                              ║
// ║  📚 DOCUMENTAÇÃO DE REFERÊNCIA (OBRIGATÓRIO LER):                            ║
// ║     docs/ADR_STAYSNET_WEBHOOK_REFERENCE.md                                    ║
// ║     docs/ADR_EDGE_FUNCTIONS_ARQUITETURA_CENTRALIZADA.md                      ║
// ║                                                                              ║
// ║  Este documento contém:                                                      ║
// ║  - Código de referência que FUNCIONA (golden copy)                           ║
// ║  - Bugs já corrigidos e como foram resolvidos                                ║
// ║  - Regras críticas de conversão de preços (REAIS → CENTAVOS)                 ║
// ║  - Queries de debug e checklist de validação                                 ║
// ║                                                                              ║
// ║  🚨 REGRAS CRÍTICAS:                                                         ║
// ║  1. parseMoneyInt() DEVE multiplicar por 100 (Stays=REAIS, Rendizy=CENTAVOS) ║
// ║  2. Usar 'staysReservationCode' (NÃO 'bookingCode' que não existe)           ║
// ║  3. JSONB lookup usa .contains() com objeto nested                           ║
// ║                                                                              ║
// ║  🔗 FUNÇÕES QUE IMPORTAM ESTE ARQUIVO:                                       ║
// ║     - staysnet-webhook-receiver (recebe webhooks em tempo real)              ║
// ║     - staysnet-webhooks-cron (processa webhooks via cron)                    ║
// ║                                                                              ║
// ║  🚀 DEPLOY OBRIGATÓRIO - Ao alterar este arquivo, DEVE rodar:                ║
// ║     npm run deploy:webhooks                                                  ║
// ║     (faz deploy de TODAS as funções dependentes automaticamente)             ║
// ║                                                                              ║
// ║  Se algo quebrar, CONSULTE O DOCUMENTO antes de tentar corrigir!             ║
// ║                                                                              ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
//
...
export async function receiveStaysNetWebhook(c: Context) {
  ...
  try {
    ...
    console.log('[StaysNet Webhook] ✅ Save succeeded, id:', save.id);
...
function parseMoneyInt(value: any, fallback = 0): number {
  ...
  if (!Number.isFinite(n)) return fallback;
  // IMPORTANTE: Stays.net envia valores em REAIS (ex: 243.96).
  // O banco armazena em CENTAVOS (INTEGER). Multiplicamos por 100.
...
async function resolveAnuncioDraftIdFromStaysId(
  ...
  // Normalizar IDs (trim + lowercase para staysId)
  const normalizedOrgId = String(organizationId || '').trim();
  const normalizedStaysId = String(staysId || '').trim();
  
  if (!normalizedStaysId) {
    console.warn('[resolveAnuncioDraftIdFromStaysId] staysId vazio ou inválido');
    return null;
  }

  const lookups: Array<{ label: string; needle: any }> = [
    { label: 'data.externalIds.staysnet_property_id', needle: { externalIds: { staysnet_property_id: normalizedStaysId } } },
    { label: 'data.externalIds.staysnet_listing_id', needle: { externalIds: { staysnet_listing_id: normalizedStaysId } } },
    { label: 'data.staysnet_raw._id', needle: { staysnet_raw: { _id: normalizedStaysId } } },
    { label: 'data.staysnet_raw.id', needle: { staysnet_raw: { id: normalizedStaysId } } },
    { label: 'data.codigo', needle: { codigo: normalizedStaysId } },
  ];
  ...
  for (const l of lookups) {
    ...
    if (row?.id) {
      console.log(`✅ [resolveAnuncioDraftIdFromStaysId] Encontrado via ${l.label}: ${row.id}`);
      return row.id;
    }
  }

  console.warn(`⚠️ [resolveAnuncioDraftIdFromStaysId] Property não encontrada para staysId=${normalizedStaysId}, orgId=${normalizedOrgId}`);
  return null;
}
...
/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║ 🚨 FUNÇÃO CRÍTICA - NÃO MODIFICAR SEM ENTENDER O CONTEXTO 🚨                ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║ Esta função extrai datas de payloads do Stays.net.                           ║
 * ║                                                                              ║
 * ║ PROBLEMA HISTÓRICO (Bug #5 - ADR_STAYSNET_WEBHOOK_REFERENCE.md):             ║
 * ║ - Webhooks de CRIAÇÃO de bloqueio usam: checkInDate, checkOutDate            ║
 * ║ - Webhooks de DELEÇÃO de bloqueio usam: from, to                             ║
 * ║                                                                              ║
 * ║ Se remover suporte a "from"/"to", bloqueios deletados no Stays               ║
 * ║ NÃO serão removidos do calendário do Rendizy!                                ║
 * ║                                                                              ║
 * ║ @see docs/ADR_STAYSNET_WEBHOOK_REFERENCE.md - Bug #5                         ║
 * ║ @author Fix aplicado em 2026-01-18                                           ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
function extractYmdRangeFromStaysLike(input: any): { startDate: string | null; endDate: string | null } {
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // ⚠️ CRÍTICO: NÃO REMOVER "from" e "to" - usados em webhooks de DELEÇÃO!
  // Stays usa formatos diferentes para criação vs deleção de bloqueios:
  //   - Criação: { checkInDate: "2026-01-28", checkOutDate: "2026-01-29" }
  //   - Deleção: { from: "2026-01-28", to: "2026-01-29" }
  // ════════════════════════════════════════════════════════════════════════════
  const start = toYmd(
    r?.checkInDate ?? r?.checkIn ?? r?.check_in ?? 
    r?.startDate ?? r?.start_date ?? 
    r?.from ??  // ← CRÍTICO para deleção de bloqueios
    null
  );
  const end = toYmd(
    r?.checkOutDate ?? r?.checkOut ?? r?.check_out ?? 
    r?.endDate ?? r?.end_date ?? 
    r?.to ??    // ← CRÍTICO para deleção de bloqueios
    null
  );
...
function mapStaysReservationToSql(
  ...
  // Soma em REAIS primeiro, depois converte para CENTAVOS
  const hostingFeesSumReais = hostingFees.reduce((acc, fee) => {
  ...
  const hostingFeesTotal = Math.round(hostingFeesSumReais * 100);  // Converter para CENTAVOS
  ...
  const resolvedBaseTotal = Number.isFinite(accommodationTotal)
    ? Math.round(accommodationTotal * 100)  // Converter REAIS para CENTAVOS
    : Number.isFinite(baseTotal)
      ? Math.round(baseTotal * 100)  // Converter REAIS para CENTAVOS
      : pricePerNight * nights;  // pricePerNight já está em centavos (parseMoneyInt)
  ...
  // IMPORTANTE: explicitTotal está em REAIS, mas computedTotal já está em CENTAVOS.
  // Se explicitTotal foi encontrado, converter para centavos (*100).
  const total = Number.isFinite(explicitTotal) ? Math.round(explicitTotal * 100) : computedTotal;

  // ============================================================================
  // COMISSÃO DA PLATAFORMA (Airbnb, Booking, etc)
  // O Stays.net envia em partner.commission._mcval.BRL (valor em REAIS)
  // ============================================================================
  const partnerObj = input?.partner || {};
  const commissionObj = partnerObj?.commission || {};
  const commissionMcval = commissionObj?._mcval || {};
  
  // Comissão em REAIS -> converter para CENTAVOS
  const platformCommissionReais = parseMoney(
    commissionMcval?.BRL ?? commissionObj?.value ?? commissionObj?.amount ?? 0,
    0,
  );
  const platformCommission = Math.round(platformCommissionReais * 100);
  
  // Nome do parceiro e tipo de comissão
  ...
  return {
    ...
    pricing_platform_commission: platformCommission,
    platform_partner_name: platformPartnerName,
    platform_commission_type: platformCommissionType,
    ...
    try {
      ...
      if (!sqlData.property_id) {
        ...
        await upsertStaysnetImportIssueMissingPropertyMapping(supabase, {
          ...
          reservationCode: staysReservationCode || null,
      ...
      if (sqlData.external_id || staysReservationCode) {
