...
// ============================================================
// DELETE /:organizationId/clear-archive
// Limpar arquivos hospedados via ZIP (modelo antigo)
// Remove o ZIP e todos os arquivos extraídos do Storage
// ============================================================
app.delete("/:organizationId/clear-archive", async (c) => {
  try {
    const { organizationId } = c.req.param();
    ...
    const { data: existing, error: fetchError } = await supabase
      .from("client_sites")
      .select("archive_path, extracted_base_url, extracted_files_count")
      .eq("organization_id", organizationId)
      .maybeSingle();
    ...
    if (!existing.archive_path && !existing.extracted_base_url) {
      return c.json(
        {
          success: false,
          error: "Este site não possui arquivos hospedados via ZIP",
        },
        400
      );
    }
    ...
    // 1. Deletar o arquivo ZIP original se existir
    if (existing.archive_path) {
      console.log(`[CLIENT-SITES] 🗑️ Deletando arquivo ZIP: ${existing.archive_path}`);
      const { error: deleteZipError } = await supabase.storage
        .from(bucketName)
        .remove([existing.archive_path]);

      if (deleteZipError) {
        console.warn(`[CLIENT-SITES] ⚠️ Erro ao deletar ZIP: ${deleteZipError.message}`);
        errors.push(`ZIP: ${deleteZipError.message}`);
      } else {
        deletedFiles++;
        console.log(`[CLIENT-SITES] ✅ ZIP deletado: ${existing.archive_path}`);
      }
    }

    // 2. Deletar todos os arquivos extraídos da pasta extracted/
    console.log(`[CLIENT-SITES] 🗑️ Listando arquivos extraídos para ${organizationId}...`);
    const extractedPrefix = `${organizationId}/extracted/`;

    // Listar arquivos em lotes (Storage retorna no máximo 1000 por vez)
    ...
    while (true) {
      const { data: files, error: listError } = await supabase.storage
        .from(bucketName)
        .list(extractedPrefix, { limit, offset });

      if (listError) {
        console.warn(`[CLIENT-SITES] ⚠️ Erro ao listar arquivos: ${listError.message}`);
      ...
      // Montar caminho completo de cada arquivo
      // Nota: Supabase Storage list() retorna objetos com 'name' (não 'id')
      const filePaths = files
        .filter(f => f.name && !f.name.endsWith('/')) // Ignorar pastas
        .map(f => `${extractedPrefix}${f.name}`);

      allExtractedFiles.push(...filePaths);

      if (files.length < limit) break;
      offset += limit;
    }

    // Deletar também arquivos em subpastas (dist/assets, etc)
    const subfolders = ['dist', 'dist/assets'];
    for (const subfolder of subfolders) {
      const subPrefix = `${organizationId}/extracted/${subfolder}/`;
      ...
      if (subFiles && subFiles.length > 0) {
        ...
        allExtractedFiles.push(...subPaths);
      }
    }

    console.log(`[CLIENT-SITES] 📋 Encontrados ${allExtractedFiles.length} arquivos para deletar`);

    // Deletar em lotes de 100 (limite do Supabase Storage)
    const batchSize = 100;
    for (let i = 0; i < allExtractedFiles.length; i += batchSize) {
      ...
      if (deleteError) {
        console.warn(`[CLIENT-SITES] ⚠️ Erro ao deletar lote: ${deleteError.message}`);
    ...
    console.log(`[CLIENT-SITES] ✅ Deletados ${deletedFiles} arquivos`);

    // 3. Limpar campos do registro no banco
    const { error: updateError } = await supabase
      .from("client_sites")
      .update({
        archive_path: null,
    ...
    if (updateError) {
      console.error(`[CLIENT-SITES] ❌ Erro ao atualizar registro: ${updateError.message}`);
    ...
    if (errors.length > 0) {
      return c.json({
        success: true,
        message: `Arquivos limpos com avisos`,
        deletedFiles,
        warnings: errors,
      });
    }
    ...
    console.error("[CLIENT-SITES] Erro ao limpar arquivo:", error);
    return c.json(
      {
        success: false,
        error: error.message,
