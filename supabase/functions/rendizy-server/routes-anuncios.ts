...
app.post("/save-field", async (c) => {
  try {
    ...
    // The editor (anuncios_ultimate.data) updates rooms, but cards won't reflect
    // until those SQL columns are updated.
    const syncCapacityToProperties = async (opts: {
      anuncioId: string;
      organizationId: string;
      anuncioData: any;
      overrideRoomsValue?: unknown;
    }) => {
      const { anuncioId, organizationId, anuncioData, overrideRoomsValue } = opts;
      ...
      const parseRooms = (v: unknown): any[] => {
        if (!v) return [];
      ...
      const rooms = parseRooms(overrideRoomsValue ?? anuncioData?.rooms);

      // Derive capacity from rooms (match frontend logic).
      let bedrooms = 0;
      let bathrooms = 0;
      let beds = 0;
      ...
      for (const room of rooms) {
        ...
        // Bedrooms: explicit room types + safe fallback.
        if (
          room?.type === "suite" ||
          room?.type === "quarto-duplo" ||
          room?.type === "quarto-individual" ||
          room?.type === "estudio" ||
          hay.includes("quarto") ||
          hay.includes("suite")
        ) {
        ...
        // Bathrooms: bathroom types OR suites (frontend counts suites as bathroom too).
        if (
          room?.type === "banheiro" ||
          room?.type === "meio-banheiro" ||
          hay.includes("banheiro") ||
          hay.includes("lavabo") ||
          room?.type === "suite" ||
          hay.includes("suite")
        ) {
        ...
        if (room?.beds && typeof room.beds === "object") {
          for (const qtd of Object.values(room.beds)) {
            beds += Number(qtd) || 0;
          }
        }
      }

      const maxGuestsRaw = anuncioData?.guests ?? anuncioData?.maxGuests ?? anuncioData?.max_guests;
      const maxGuests = Number(maxGuestsRaw);
      ...
      if (!Number.isNaN(maxGuests)) {
        updatePayload.max_guests = maxGuests;
      }
      ...
      try {
        ...
        if (updateError) {
          console.warn("⚠️ [save-field] sync properties failed:", updateError);
        } else {
          console.log("✅ [save-field] synced capacity to properties:", {
            anuncioId,
            bedrooms,
            bathrooms,
            beds,
        ...
        console.warn("⚠️ [save-field] sync properties threw:", syncErr);
      }
    };

    // Only sync when it can affect card stats.
    try {
      if (result?.id && organizationId) {
        const fieldLower = String(field || "").toLowerCase();
        if (fieldLower === "rooms" || fieldLower === "guests" || fieldLower === "max_guests" || fieldLower === "maxguests") {
          await syncCapacityToProperties({
            anuncioId: result.id,
            organizationId,
            anuncioData: result.data,
            overrideRoomsValue: fieldLower === "rooms" ? value : undefined,
          });
        }
      }
    } catch (e) {
      console.warn("⚠️ [save-field] sync properties skipped due to error:", e);
    }
...
app.patch('/:id', async (c) => {
  try {
    ...
    // Sync derived capacity fields into `properties` after a full data PATCH.
    try {
      const normalize = (v: unknown) =>
        String(v ?? "")
          .toLowerCase()
      ...
      const rooms = parseRooms(data.data?.rooms);
      if (rooms.length > 0) {
        let bedrooms = 0;
        let bathrooms = 0;
        let beds = 0;
        ...
        const maxGuestsRaw = data.data?.guests ?? data.data?.maxGuests ?? data.data?.max_guests;
        const maxGuests = Number(maxGuestsRaw);
        const updatePayload: any = {
          updated_at: new Date().toISOString(),
          bedrooms,
        ...
        if (updateError) {
          console.warn("⚠️ [PATCH /:id] sync properties failed:", updateError);
        } else {
          console.log("✅ [PATCH /:id] synced capacity to properties:", {
            anuncioId: id,
            bedrooms,
            bathrooms,
            beds,
      ...
      console.warn("⚠️ [PATCH /:id] sync properties threw:", syncErr);
    }
