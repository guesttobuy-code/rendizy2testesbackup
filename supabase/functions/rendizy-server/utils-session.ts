...
// ✅ CORREÇÃO CRÍTICA: Tokens legados (ex.: "micjk8ts_qffa7w735o_...") tinham ~30 caracteres e não existem mais
  ...
  try {
    ...
    // ✅ ARQUITETURA OAuth2 v1.0.103.1010: Verificar expiração da sessão
    // ✅ CORREÇÃO v1.0.103.1020: Usar expires_at como referência principal (7 dias)
    // O access_expires_at curto (30min) causava 401 desnecessário no fluxo atual
    const now = new Date();
    
    // ✅ VERIFICAR expires_at principal (7 dias de validade)
    const expiresAt = new Date(sessionRow.expires_at);
    if (now > expiresAt) {
      console.log('⚠️ [getSessionFromToken] Sessão expirada (expires_at)');
      return null;
    }
    
    // ✅ LOG: Se access_expires_at expirou mas sessão ainda válida, continuar
    if (sessionRow.access_expires_at) {
      const accessExpiresAt = new Date(sessionRow.access_expires_at);
      if (now > accessExpiresAt) {
        console.log('ℹ️ [getSessionFromToken] Access token curto expirado, mas sessão principal ainda válida');
        // ✅ Não retornar null - sessão ainda é válida pelo expires_at
      }
    }
