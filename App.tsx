...
function App() {
  ...
  */
  ...
  const runCalendarRulesBatch = async (operations: any[]) => {
    ...
    // ✅ Migrado para rendizy-server (ADR Edge Functions Centralizadas)
    const edgeUrl = `https://${projectId}.supabase.co/functions/v1/rendizy-server/calendar-rules/batch`;
    ...
    for (let i = 0; i < operations.length; i += MAX_BATCH) {
      ...
      if (savedIds.length > 0) {
        ...
        // ✅ Migrado para rendizy-server (ADR Edge Functions Centralizadas)
        const verifyUrl = `https://${projectId}.supabase.co/functions/v1/rendizy-server/calendar-rules/batch?ids=${encodeURIComponent(idsParam)}`;
    ...
    const loadReservations = async () => {
      ...
      try {
        ...
        if (reservationsResponse.success && reservationsResponse.data) {
          const loadedGuests = normalizeList(guestsResponse.data ?? guestsResponse);
          setGuests(loadedGuests);

          // Convert API reservations to App Reservation format
          // ✅ CORREÇÃO: Preservar TODOS os dados da API, incluindo pricing completo e guests
          const apiReservations = reservationsResponse.data.map((r: any) => {
            ...
            return {
              ...r, // ✅ PRESERVAR todos os campos da API (pricing, guests, notes, etc)
              id: r.id,
              propertyId: r.propertyId,
              guestId: r.guestId,
              guestName: guest ? guest.fullName : r.guestName || 'Hóspede',
              ...
              price: r.pricing?.total ?? 0, // ✅ Mantém compatibilidade
              pricing: r.pricing, // ✅ PRESERVAR objeto pricing completo
              guests: r.guests,   // ✅ PRESERVAR objeto guests completo
              nights: r.nights,
              notes: r.notes,
              payment: r.payment,
            };
                  ...
                  <ProtectedRoute>
                    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
                      <MainSidebar
                        collapsed={sidebarCollapsed}
                        onToggleCollapse={() => setSidebarCollapsed(!sidebarCollapsed)}
                        activeModule="usuarios-usuarios"
                        onModuleChange={setActiveModule}
                        onSearchReservation={handleSearchReservation}
                        onAdvancedSearch={handleAdvancedSearch}
                      />
